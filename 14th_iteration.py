...
# Defining different regression models to test
models = {
...
if uploaded_file is not None:
    ...
    try:
        ...
        user_mse = mean_squared_error(user_data['Predicted_Traffic_Volume'], user_predictions)
        user_r2 = r2_score(user_data['Predicted_Traffic_Volume'], user_predictions)
        user_rmse = np.sqrt(user_mse)
        user_adj_r2 = 1 - (1 - user_r2) * (user_data.shape[0] - 1) / (user_data.shape[0] - user_data.shape[1] - 1)

        # Define the metrics to be used in the chart
        metrics = ['RMSE', 'Adjusted R²']  # Define the metrics list


  # Display additional metrics for user data
        st.write(f"Root Mean Squared Error (RMSE) for User Data: {user_rmse}")
        st.write(f"Adjusted R² for User Data: {user_adj_r2}")

        # Bar Graph for additional metrics: RMSE and Adjusted R² for user data (Updated layout)
        st.subheader("User Data Performance Metrics (RMSE & Adjusted R²)")
        user_scores = [user_rmse, user_adj_r2]    

        plt.figure(figsize=(10, 6))
        plt.barh(metrics, user_scores, color='green')
        plt.xlabel("Metric Value")
        plt.ylabel("Metric")
        plt.title("User Data Performance Metrics (RMSE & Adjusted R²)")
        st.pyplot(plt)

    except Exception as e:
        # If an error occurs during prediction, display it
